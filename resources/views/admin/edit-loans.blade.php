        ...
        <div class="page-wrapper">
            <div class="page-content">
                ...
                <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                    <div class="breadcrumb-title pe-3">Edit Loan</div>
                    <div class="ps-3">
                ...
                <div class="row">
                    <div class="col-xl-6 mx-auto">
                        <h6 class="mb-0 text-uppercase" style="text-align: center ">Edit {{ $loans->firstname }} Loan
                            Request</h6>
                        <hr />
                        <div class="card border-top border-0 border-4 border-primary">
                            <div class="card-body p-5">
                                <form class="row g-3" method="POST" data-post-type="edit-plan"
                                    action="{{ route('admin.loan.view', ['edit', $loans->id]) }}">
                                    ...
                                    <div class="col-md-12">
                                        <label for="inputLastName" class="form-label">Loan Amount
                                            ({{ $loans->currency }})</label>
                                        <input required="" type="text" name="amount" class="form-control" id="amount"
                                            placeholder="Enter amount"
                                            value="{{ !empty(old('amount')) ? old('amount') : number_format($loans->amount, 0, '', '') }}">
                                    </div>
                                    @error('amount')
                                        <span class="text-danger" id="error_amount">{{ $message }}</span>
                                    @enderror
                                    <div class="col-md-12">
                                        <label for="inputState" class="form-label">Loan Status</label>
                                        <select id="inputState" name="status" class="form-select  form-control">
                                            <option value="" disabled selected>Select</option>
                                            @foreach (config('app.deposit_status') as $key => $status)
                                                <option @if (old('status') == $key || $loans->status == $key) selected @endif
                                    ...
                                    @enderror
                                    <div class="col-md-12">
                                        <button type="submit" name="edit-plan" class="btn btn-primary px-5 w-100 form-control">Edit Loan
                                            Record</button>
                                    </div>
                                </form>
                            </div>
